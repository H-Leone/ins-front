...
function ResearchCard({ id, title, status, scheduledDate, form }: IResearch) {
  return (
      ...
      <div className="px-12 py-6 border-solid border border-[#8A8A8E] rounded-lg flex justify-between items-center gap-2 shadow-md">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-insightfy-gray font-semibold">
            {form.map((e) => e.responses.length).length} respostas
          </p>
        </div>

        <span
          style={{ backgroundColor: researchStatusColor(status) }}
          className={`w-48 h-8 flex gap-2 justify-center items-center text-sm font-semibold rounded-lg ${
        ...
        >
          <p className="text-center font-medium">
            {getStatusText(status)} {getSufix(status)}{" "}
